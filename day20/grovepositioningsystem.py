...
"""grovepositioningsystem.py: Solution to Advent of Code 2022 Day 20, Part 1
...
for line in input:
    tail = CoordinateNode(int(line),prev,None)
    prev.next = tail
    prev.ordernext = tail
    prev = tail
...
#    print(curr.coordinate)
    if curr.coordinate > 0:
        #move forward
        target = curr
        #TODO: problem when target is supposed to == curr
        for i in range(curr.coordinate):
            target = target.next
